...
function greetUser(senderID) {
    sendTwilioMessage("Hi :) \nI am Rosy your workorder assistant. You can start a new workorder by sending\n#workorder", senderID);
}

function sendTwilioMessage(message_to_send,senderID)
{
    //IML
    //var twiml = new twilio.TwimlResponse();
    //twiml.message(message_to_send);
...
function handleNlpResponse(action, context, parameter, inputText, senderID) 
{
    if (action == "send_greetings") {
        ...
        sendTwilioMessage("I am all geared up to help you. Please provide your phone number.", senderID);
        ...
        if (cache_entry != undefined) {
            ...
            sendTwilioMessage("Sorry ! These details are not registered with us.", senderID);
        }
    } else 
    {
        ...
        if (cache_entry != undefined) 
        {
            ...
            dbconn.query('SELECT * FROM workorder where status = 0 AND userid="' + cache_entry[0] + "##" + cache_entry[1] + "##" + cache_entry[2] + '"', function(err, records) {
                ...
                if (diffHours < 4) // 4 hour check
                {
                    ...
                    {
                        //Update task
                        sendTwilioMessage("Is it an emergency? YES or NO", senderID);
                        dbconn.query(
                        ...
                        sendTwilioMessage("Which date you are comfortable with?", senderID);
                        ...
                        listEventsForDate(getCalendarToken(), startDate, endDate, records[0].id, function(returnVal, startDate, id) {
                            ...
                            if (slots > 0) {
                                ...
                                //sendTwilioMessage("Date:" + startDate.toLocaleDateString() + "\nChoose a slot:\n" + slots_to_show + "\n\nReply with SLOT <slot number>", senderID);
                                sendTwilioMessage("\nChoose a slot:\n" + slots_to_show + "\n\nReply with SLOT <slot number>", senderID);

                            } else
                                sendTwilioMessage("Sorry :( No slots available for the selected date!\n\nPlease choose another day!", senderID);
                        ...
                        var slot_index = "";
                        slot_index = parseInt(tokens[1]) - 1;
                        ...
                        setCalendarEntry(getCalendarToken(), records[0].userid.split("##")[1], start_date1, end_date1, records[0].id, message, records[0].task, function(returnVal) {
                            if (returnVal == null) {
                                sendTwilioMessage("Sorry :( We are unable to process your request. Please try later.", senderID);
                            } else {
                                sendTwilioMessage("Awesome! Your work request has been registered with us. You will receieve and email shortly.", senderID);
                    ...
                    dbconn.query('delete FROM workorder1 where status = 0 AND userid="' + cache_entry[0] + "##" + cache_entry[1] + "##" + cache_entry[2] + '"', function(err, res) {
                        if (err)
                            throw err;
                        console.log('Last record insert id:', res.insertId);
                        sendTwilioMessage("Session expired!\nTry #workorder", senderID);
                    });
                }
            });
        } else {
                    ...
                    if (inputText.toUpperCase().includes("YES") == true) {
                        dbconn.query(
                            ...
                            function(err, result) {
                                if (err) throw err;
                                console.log('Changed ' + result.changedRows + ' rows');
                                sendTwilioMessage("I am glad. I was able to help you. Let me know if I can help further by sending #workorder", senderID);
                        ...
                        var curr_date = moment().format('YYYY-MM-DD HH:mm:ss');
                         dbconn.query(
                            ...
                            function(err, result) {
                                ...
                                sendTwilioMessage("Oh! Let me reschedule it for you. Which date are you comforatble with?", senderID);
                                delete follow_up_arr[records[0].user_id];
                            });
                    }
                } else {
                    sendTwilioMessage("EWWWWWW. I will be honest but at last I am a mere bot. Try #workorder", senderID);
                }
            });
    ...
    dbconn.query('SELECT * FROM workorder where status = 1 AND appointment_time < "' + moment().format('YYYY-MM-DD HH:mm:ss') + '"', function(err, records) {
        ...
        for (var i = 0; i < records.length; i++) {
            if (follow_up_arr[records[i].userid] == undefined) {
                ...
                sendTwilioMessage("Your workorder '" + records[i].task + "' was scheduled for " + records[i].appointment_time + ". Is it completed now? YES/NO", records[i].fb_token);
